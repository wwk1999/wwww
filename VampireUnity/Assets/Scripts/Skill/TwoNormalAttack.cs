...
public class TwoNormalAttack : MonoBehaviour
{
    ...
    private void OnTriggerEnter2D(Collider2D other)
    {
        if (other.CompareTag("Monster")||other.CompareTag("Boss"))
        {
            ...
            other.transform.parent.GetComponent<MonsterBase>().Hurt(QueueController.S.GameAttack*1.5f*SkillController.S.HeiAnYuanSuDamage,isCrit,DamageFrom.NormalAttack,YuanSuType.HeiAn);
    ...
    private void OnTriggerStay2D(Collider2D other)
    {
        if (other.CompareTag("Monster")||other.CompareTag("Boss"))
        {
            ...
            if (CurrentTime >= HitTime)
            {
                CurrentTime = 0;
                bool isCrit = GameController.S.GetIsCrit();
                QueueController.S.MonsterColliderDic[other].Hurt(QueueController.S.GameAttack*1.5f*SkillController.S.HeiAnYuanSuDamage,isCrit,DamageFrom.NormalAttack,YuanSuType.HeiAn);
            }
        }
